...
full_model_twoway <- glm(chd69 ~ (agegroup + sbp10 + cholmmol + bmi + smokerf + arcus0 + dibpat0f)^2, 
                  family = binomial, data = di)

full_model_threeway <- glm(chd69 ~ (agegroup + sbp10 + cholmmol + bmi + smokerf + arcus0 + dibpat0f)^3, 
                  family = binomial, data = di)

#full_model <- glm(chd69 ~ (agegroup*sbp10*cholmmol*bmi*smokerf*arcus0*dibpat0f), 
                  #family = binomial, data = di)
...
# Testing the models
test_model <- glm(chd69 ~ agegroup + sbp10 + cholmmol + bmi + smokerf + dibpat0f + cholmmol:bmi, family = binomial, data = di)
test_model2 <- glm(chd69 ~ agegroup + sbp10 + cholmmol + bmi + smokerf + arcus0 + dibpat0f + agegroup:bmi + agegroup:smokerf + sbp10:bmi + sbp10:arcus0 + cholmmol:bmi, family = binomial, data = di)
#test_model3 <- glm(chd69 ~ agegroup + sbp10 + cholmmol + bmi + smokerf + arcus0 + dibpat0f + agegroup:bmi + agegroup:smokerf + sbp10:bmi + sbp10:arcus0 + cholmmol:bmi+ cholmmol:smokerf + cholmmol:arcus0 + smokerf:arcus0 + sbp10:smokerf:arcus0 +cholmmol:smokerf:arcus0, family = binomial, data = di)

# Likelihood ratio test
anova(test_model, test_model2, test = "Chisq")

optimal_model <- glm(chd69 ~ agegroup + sbp10 + cholmmol + bmi + smokerf + arcus0 + dibpat0f, family = binomial, data = di)
#summary(optimal_model)

optimal_model2 <- glm(chd69 ~ agegroup + sbp10 + cholmmol + bmi + smokerf + arcus0 + dibpat0f + sbp10:bmi + sbp10:arcus0 + cholmmol:bmi, family = binomial, data = di)
#summary(optimal_model2)

optimal_model3 <- glm(chd69 ~ agegroup + sbp10 + cholmmol + bmi + smokerf + arcus0 + dibpat0f + agegroup:bmi + agegroup:smokerf + sbp10:bmi + sbp10:arcus0 + cholmmol:bmi, family = binomial, data = di)
#summary(optimal_model3)

optimal_model4 <- glm(chd69 ~ agegroup + sbp10 + cholmmol + bmi + smokerf + dibpat0f + cholmmol:bmi, family = binomial, data = di)
#summary(optimal_model4)
```
...
numeric_vars <- di %>% select(age0, sbp10, cholmmol, bmi)  # Select numeric predictors
cor_matrix <- cor(numeric_vars, use = "complete.obs")
corrplot(cor_matrix, method = "circle", type = "lower", tl.col = "black", tl.cex = 0.8)


#vif(optimal_model)
#interaction.plot(di$agegroup,di$smokerf,di$chd69,fixed=TRUE)
# Scatter plot of sbp10 vs. cholmmol
...
#di$predicted_risk <- predict(optimal_model, type = "response")
predicted_risk <- predict(optimal_model3, type = "response")
...
ggplot(roc_data, aes(x = 1 - Specificity, y = Sensitivity)) +
  ...
  labs(
    ...
    subtitle = "Using Logistic Regression/Optimal model 2",
...
# Fit the logistic regression model using the rms package
logit_model <- lrm(
  chd69 ~ agegroup + sbp10 + cholmmol + bmi + smokerf + arcus0 + dibpat0f +
    agegroup:bmi + agegroup:smokerf + sbp10:bmi + sbp10:arcus0 + cholmmol:bmi,
  data = di,
  x = TRUE, y = TRUE
)
# Perform optimism adjustment using bootstrap validation (200 repetitions)

validate_result <- validate(logit_model, B = 200)  # 200 bootstrap repetitions
adjusted_auc <-  validate_result['Dxy', 'index.corrected'] / 2  + 0.5 # Convert Dxy to AUC, formula from wiki
...
folds <- sample(1:10, nrow(di), replace = TRUE)  # Assign folds
...
hl_test <- hoslem.test(x = di$chd69, y = fitted(optimal_model3), g =10)
...
# Example: Count the number of rows for each level in the "smokerf" column
di %>% count(smokerf,agegroup,arcus0)
...
# Plot the Decision Curve with bolder lines
plot(dca_results, col =colors,smooth =TRUE, lwd = 1.5) +  # Increase line width using lwd
  labs(
...
# Plot the Decision Curve with bolder lines
plot(dca_results_comparison, col =colors,smooth =TRUE, lwd = 1.5) +  # Increase line width using lwd
  labs(
    title = "Decision Curve Analysis",
    subtitle = "Comparing Net Benefit of Optimal Model vs Default Strategies",
    x = "Threshold Probability",
...
```{r test}
library(splines)
# Scatterplot with smoothed curve for a continuous predictor
ggplot(di, aes(x = sbp10, y = chd69)) +
  geom_point(alpha = 0.5) +  # Points
  geom_smooth(method = "loess", se = TRUE, color = "blue") +  # Smoothed curve
  labs(title = "Relationship between SBP and CHD Risk", x = "SBP (scaled by 10)", y = "CHD (0/1)") +
  theme_minimal()

# Fit models with linear and spline terms
linear_model <- glm(chd69 ~ sbp10, family = binomial, data = di)
spline_model <- glm(chd69 ~ ns(sbp10, df = 3), family = binomial, data = di)

# Compare models
anova(linear_model, spline_model, test = "Chisq")
AIC(linear_model, spline_model)

test_model <- glm(chd69 ~ agegroup + sbp10 + cholmmol + I(sbp10^2) + bmi + smokerf + arcus0 + dibpat0f + agegroup:bmi + agegroup:smokerf + sbp10:bmi + sbp10:arcus0 + cholmmol:bmi, family = binomial, data = di)

# Compare models using LRT
anova(optimal_model3, test_model, test = "Chisq")
...
rf_model <- randomForest(chd69 ~ agegroup + sbp10 + cholmmol + bmi + smokerf + arcus0 + dibpat0f, 
                         data = di, 
...
rf_probs <- predict(rf_model, di, type = "prob")[, 2]  # Probabilities for class 1

# Calculate AUC for Random Forest
rf_roc <- roc(di$chd69, rf_probs)
rf_auc <- auc(rf_roc)
cat("Random Forest AUC:", rf_auc, "\n")
# Compare ROC curves
roc_test <- roc.test(rf_roc, roc_curve, method = "delong")
cat("P-value for AUC Comparison:", roc_test$p.value, "\n")
```{r test}

```
```{r test class imbalance}

table(di$agegroup, di$chd69)
prop.table(table(di$agegroup, di$chd69), margin = 1)  # Row-wise proportions


weights <- ifelse(di$chd69 == 1, 1 / sum(di$chd69 == 1), 1 / sum(di$chd69 == 0))
weighted_model <- glm(chd69 ~ agegroup + sbp10 + cholmmol + bmi + smokerf + arcus0 + dibpat0f, 
                      family = binomial, data = di, weights = weights)
summary(weighted_model)

weighted_model2 <- glm(chd69 ~ agegroup + sbp10 + cholmmol + bmi + smokerf + arcus0 + dibpat0f + agegroup:bmi + agegroup:smokerf + sbp10:bmi + sbp10:arcus0 + cholmmol:bmi, family = binomial, data = di, weights = weights)
summary(weighted_model2)
# Compare models using LRT
anova(weighted_model, weighted_model2, test = "Chisq")
