...
full_model <- glm(chd69 ~ (agegroup + sbp10 + cholmmol + bmi + smokerf + arcus0 + dibpat0f)^2, 
                  family = binomial, data = di)

# Stepwise regression AIC
step_modelAIC <- step(full_model, direction = "both", k = 2, trace=0)
summary(step_modelAIC)

# Stepwise regression BIC
step_modelBIC <- step(full_model, direction = "both", k = log(nrow(di)),trace=0)
summary(step_modelBIC)

# Testing the models
test_model <- glm(chd69 ~ agegroup + sbp10 + cholmmol + bmi + smokerf + arcus0 + dibpat0f, family = binomial, data = di)
...
di <- di %>% mutate(predicted_risk_model = predicted_risk)
# Display the first few rows of the dataset with the predicted risk
head(di)
```
...
```{r Cross-validaiton}
if (!require("cvAUC")) install.packages("cvAUC")
library(cvAUC)
